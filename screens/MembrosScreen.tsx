...
type Membro = {
  id: string;
  nome: string; sobrenome: string; data_nascimento: string;
...
const EMPTY: Omit<Membro, 'id'> = {
  nome: '', sobrenome: '', data_nascimento: '', nacionalidade: 'Brasileira',
...
// ─── Form Modal ───────────────────────────────────────────────────────────────
function MembroFormModal({ visible, membro, onClose, onSaved }: {
  visible: boolean; membro: Membro | null;
  ...
  const SECTIONS = ['Pessoal', 'Contato', 'Endereço', 'Igreja'];
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
              ...
              {section === 2 && (
                <View style={fm.sectionContent}>
                  ...
                  <Field label="CEP" value={form.cep} onChangeText={set('cep')} placeholder="00000-000" keyboardType="numeric" maxLength={9} />
                </View>
              )}
              {section === 3 && (
                <View style={fm.sectionContent}>
...
// ─── Detail Modal ─────────────────────────────────────────────────────────────
function MembroDetailModal({ membro, onClose, onEdit, onDelete }: {
  membro: Membro | null; onClose: () => void; onEdit: () => void; onDelete: () => void;
}) {
